...
export async function POST(request) {
  ...
  const { uploadId, key, uploadJobId, parts, assetId, checksum, name, type, folder, tags } = body ?? {};
  if (!uploadId || !key || !uploadJobId || !Array.isArray(parts) || parts.length === 0) {
    return NextResponse.json({ error: "bad_request" }, { status: 400 });
  }
  ...
  const completed = await completeMultipart(key, uploadId, parts);
  if (!completed) return NextResponse.json({ error: "complete_failed" }, { status: 400 });
