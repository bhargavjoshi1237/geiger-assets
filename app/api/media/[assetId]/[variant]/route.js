...
export async function GET(request, { params }) {
  try {
    ...
    // CDN seam. A derivative is content-addressed by (asset, variant, format)
    // and rewritten rather than mutated, so it is safe to hand the client an
    // origin URL instead of carrying the bytes through Node. Null unless an
    // operator configured a CDN on the backend, so an unconfigured deployment
    // proxies exactly as it did. Access was authorized above — what a CDN hop
    // gives up (re-checked permissions, byte metering) is spelled out in
    // lib/storage/cdn.js. The original fallback above deliberately does NOT
    // redirect: its URL is stable across a re-upload, so a CDN copy would pin
    // superseded bytes.
    ...
    if (cdn) {
      ...
      // The negotiated format chose which derivative this resolved to, so the
      // redirect itself has to vary on Accept — otherwise a shared cache sends
      // one client to another client's format.
