...
export async function listCollections(projectId) {
  ...
  if (projectId && !isUuid(projectId)) {
    console.warn("[collections.list] skipping fetch: invalid projectId", projectId);
    return [];
  }
  try {
...
export async function addAssetToCollection(collectionId, assetId, position) {
  ...
  try {
    ...
    if (!Number.isFinite(nextPosition)) {
      const { count } = await sb
        ...
        .eq("collection_id", collectionId);
      nextPosition = Number(count ?? 0);
    }
    const { data, error } = await sb
      .from(JOIN_TABLE)
      .insert({ collection_id: collectionId, asset_id: assetId, position: nextPosition })
      .select("*, asset:asset_id(id,name,type,format,color,status,size_bytes)")
      .single();
    if (error) {
