...
//   publicUrl(key)               -> url | null            (optional, sync)
//   health()                     -> { ok, detail }
//
...
//
// publicUrl is the CDN seam: a backend fronted by a CDN returns the origin-side
// URL for a key, and a delivery route can redirect to it instead of streaming
// the bytes through Node. It is deliberately SYNCHRONOUS -- it is pure string
// work, and making it async would put an await on the hot path for a concat.
// It returns null when no CDN origin is configured, which is the default, so
// every existing deployment keeps proxying exactly as it did.
...
export const CAPABILITIES = Object.freeze([
  "presignedReads",
  "presignedWrites",
  // True only when a CDN/public origin is configured for this backend, i.e.
  // publicUrl() can actually answer. Never inferred from the bucket being
  // public -- an operator has to name the origin.
  "publicReads",
  "multipart",
  "range",
  "list",
...
export function sealBackend(backend) {
  ...
  // publicUrl is sealed separately because it is synchronous: routing it
  // through the async wrapper below would hand callers a Promise where the
  // contract promises a string, and every call site is on a delivery hot path.
  if (typeof backend.publicUrl === "function") {
    sealed.publicUrl = (key, ...rest) => {
      // Guarded here, not just in each driver: a driver that forgets the check
      // would return the bare CDN origin for an empty key, which is a valid URL
      // pointing at the wrong thing. Sealing is the place that cannot be
      // forgotten.
      if (!key) return null;
      try {
        return backend.publicUrl(key, ...rest) || null;
      } catch (e) {
        console.error(`[storage.backend.${backend.kind}.publicUrl]`, e?.message || e);
        return null;
      }
    };
  } else {
    sealed.publicUrl = () => null;
  }
