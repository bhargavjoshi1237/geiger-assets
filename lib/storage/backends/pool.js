...
// too_large.
function mapPoolError(stage, status, payload) {
  const code = payload?.code;
  console.error("[storage.pool]", {
    stage,
    status,
    code: code || null,
    message: String(payload?.error || "Pool request failed.").slice(0, 500),
  });
...
export const backend = {
  ...
  async put({ key, body, contentType }) {
    ...
    try {
      ...
      if (!res.ok) {
        mapPoolError("direct_upload", res.status, payload);
        return null;
      }
      return {
  ...
  async issueUpload({ key, contentType, sizeBytes }) {
    ...
    try {
      ...
      const payload = await readJsonSafe(res);
      if (!res.ok) return mapPoolError("reservation", res.status, payload);
      return {
        mode: payload.mode,
  ...
  async finalizeUpload({ uploadRef, parts, response, providerKey } = {}) {
    ...
    try {
      ...
      const payload = await readJsonSafe(res);
      if (!res.ok) return mapPoolError("commit", res.status, payload);
      return {
