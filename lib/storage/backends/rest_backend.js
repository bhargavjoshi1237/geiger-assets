...
export function createRestBackend(input) {
  ...
  // A CDN in front of the pod, if there is one. Falls back to publicBaseUrl so
  // an existing REST backend that already names a public read origin gains the
  // CDN seam without being reconfigured. Either way, objects served from it
  // bypass requireProjectAccess -- see lib/storage/cdn.js.
  const cdnBase = cfg.cdnBaseUrl ? stripTrailingSlash(cfg.cdnBaseUrl) : publicBase;
  ...
  const backend = {
    ...
    // Origin-side URL for a key, or null when neither a CDN nor a public read
    // origin is configured. Used only for immutable derivative keys.
    publicUrl(key) {
      if (!key || !cdnBase) return null;
      return `${cdnBase}/${encodeKeySegments(key)}`;
    },
