...
import { writeTarget, driverFor, locateObject, mirrorWrite, invalidateKey } from "@/lib/storage/pool";
...
export async function upsertUploadJob({ uploadJobId, projectId, filename, fileType, sizeBytes, storageKey, mode, status, backendId }) {
  try {
    const sb = await serverDb();
    const payload = {
      id: uploadJobId,
      ...
      upload_mode: mode,
      // Which backend the bytes are being written to. The commit runs in a
      // separate request on the presigned path, so the placement decision has
      // to survive on the job row -- re-deciding it at commit time could pick a
      // different pool member than the one actually holding the object.
      storage_backend: backendId ?? null,
    };
...
// Recovers which backend a presigned upload was placed on. Returns null both
// for "the env default" and for "could not tell" -- and null is the correct
// answer in the second case too, since a deployment with no pool configured
// has nowhere else the bytes could be.
async function backendForJob(uploadJobId) {
  if (!uploadJobId) return null;
  try {
    const sb = await serverDb();
    const { data } = await sb.schema("assets").from("upload_jobs")
      .select("storage_backend").eq("id", uploadJobId).single();
    return data?.storage_backend ?? null;
  } catch (e) {
    console.error("[storage.backendForJob]", e?.message || e);
    return null;
  }
}
...
export async function issueUploadUrl({ projectId, assetId, filename, contentType, sizeBytes, uploadJobId }) {
  ...
  // Placement happens here, before a single byte moves, so the URL the client
  // is about to upload to and the backend recorded on the job are the same
  // decision rather than two that could drift apart.
  const target = await writeTarget({ projectId, sizeBytes: size });
  if (!target) return { error: "storage_unconfigured" };

  // Presigning is a per-backend capability, not a deployment-wide one: a pool
  // can hold an S3 bucket that presigns beside a REST pod that cannot, and the
  // client has to be told which mode this particular upload is using.
  if (!target.driver.capabilities.presignedWrites) {
    await upsertUploadJob({
      uploadJobId, projectId, filename, fileType, sizeBytes: size,
      storageKey: key, mode: "proxy", status: "uploading", backendId: target.backendId,
    });
    return { url: null, key, mode: "proxy", expiresAt: null };
  }

  const url = await target.driver.signPut(key, { ttl: cfg.uploadUrlTtl, contentType, maxBytes: size });
  if (!url) return { error: "sign_failed" };

  await upsertUploadJob({
    uploadJobId, projectId, filename, fileType, sizeBytes: size,
    storageKey: key, mode: "presigned", status: "uploading", backendId: target.backendId,
  });
...
function toAssetInsert({ projectId, key, head, checksum, name, type, folder, tags, filename, contentType, backendId, bucket }) {
  return {
    project_id: projectId,
    name: name || filename,
    ...
    storage_key: key,
    storage_bucket: bucket || s3Config().bucket,
    // null means the env-configured default, which is what every row written
    // before pooling existed carries -- so they keep resolving untouched.
    storage_backend: backendId ?? null,
...
export async function commitUpload({ uploadJobId, key, assetId, checksum, name, type, folder, tags, bytes, probe, backendId }) {
  if (!key || !uploadJobId) return { error: "bad_request" };
  const parsed = parseKey(key);
  if (!parsed?.projectId) return { error: "bad_key" };

  // On the proxy path the caller already knows where it wrote. On the
  // presigned path the commit is a separate request, so the placement made at
  // sign time is recovered from the job row rather than guessed at.
  const placedOn = backendId !== undefined ? backendId : await backendForJob(uploadJobId);
  const driver = await driverFor(placedOn);
  if (!driver) {
    await markJob(uploadJobId, { status: "failed", error: "Storage backend unavailable" });
  ...
  try {
    ...
    if (assetId) {
      ...
      const { data, error } = await sb.schema("assets").from("assets").update({
        storage_key: key,
        storage_bucket: driver.bucket || cfg.bucket,
        storage_backend: placedOn ?? null,
        storage_status: "stored",
        etag: head.etag,
        ...(checksum ? { checksum } : {}),
      ...
      const { data, error } = await sb.schema("assets").from("assets").insert(
        toAssetInsert({
          projectId, key, head, checksum, name, type, folder, tags, filename,
          contentType: head.contentType, backendId: placedOn, bucket: driver.bucket,
        })
      ).select("*").single();
      if (error) {
        console.error("[storage.commit]", error.message);
...
export async function proxyStore({ projectId, uploadJobId, filename, contentType, bytes, assetId, name, type, folder, tags, checksum, quality }) {
  ...
  // Placement runs on the post-optimisation size, so a pod whose per-object
  // ceiling the original would have blown is still eligible once the re-encode
  // has brought the file under it.
  const target = await writeTarget({ projectId, sizeBytes: size });
  if (!target) return { error: "storage_unconfigured" };
  ...
  const stored = await target.driver.put(payload);
  if (!stored) {
    await markJob(uploadJobId, { status: "failed", error: "Proxy upload failed" });
    return { error: "put_failed" };
  }
  // Under `mirror` the remaining members get the same bytes, best-effort. The
  // primary copy is already durable, so a pod that is down delays redundancy
  // rather than failing the upload.
  mirrorWrite(target.mirrors, payload);
  ...
  return commitUpload({
    ...
    bytes: effectiveBytes, probe: storedProbe, backendId: target.backendId,
  });
}
  ...
  try {
    ...
    if (parsed.assetId) {
      const { data } = await sb.schema("assets").from("assets")
        .select("id, project_id, storage_backend").eq("id", parsed.assetId).is("deleted_at", null).single();
      if (!data || (data.project_id && data.project_id !== parsed.projectId)) return null;
      parsed.backendId = data.storage_backend ?? null;
    } else if (parsed.staging && parsed.uploadJobId) {
      const { data } = await sb.schema("assets").from("upload_jobs")
        .select("id, project_id, storage_backend").eq("id", parsed.uploadJobId).single();
...
export async function batchSign(keys, sessionUserId) {
  ...
  const base = process.env.NEXT_PUBLIC_BASE_PATH || "";
  const proxyUrl = (key) => `${base}/api/storage/file?key=${encodeURIComponent(key)}`;
  let anySigned = false;
  const resolved = await Promise.all(
    list.map(async (key) => {
      const ok = await authorizeKey(key, sessionUserId);
      if (!ok) return [key, null];
      // Per key rather than per deployment: one pool can hold an S3 bucket that
      // presigns next to a REST pod that cannot, so the mode is a property of
      // where this particular object lives.
      const driver = await driverFor(ok.backendId ?? null);
      if (!driver) return [key, null];
      if (!driver.capabilities.presignedReads) return [key, proxyUrl(key)];
      const signed = await driver.signGet(key);
      if (!signed) return [key, proxyUrl(key)];
      anySigned = true;
      return [key, signed];
    })
  );
  for (const [key, url] of resolved) urls[key] = url;
  // Only the signed URLs expire; proxy URLs are valid as long as the session
  // is. The earliest expiry is the honest answer for the batch as a whole.
  const ttl = cfg?.signedUrlTtl ?? 3600;
  return { urls, expiresAt: anySigned ? new Date(Date.now() + ttl * 1000).toISOString() : null };
}
...
// Picks the driver that can actually serve this key.
//
// The asset row names the backend it was written to, which is the answer in
// every ordinary case. locateObject is the recovery path: it walks the rest of
// the pool, which is what makes a `mirror` pool survive losing a member and
// what keeps objects readable while they are being migrated off a retired one.
async function readDriver(key, { row, backendId } = {}) {
  if (backendId !== undefined) return driverFor(backendId);
  if (!row) return driverFor(null);
  const located = await locateObject(row, key);
  return located?.driver ?? (await driverFor(row.storage_backend ?? null));
}

export async function proxyStreamResponse(key, { filename, download, request, row, backendId } = {}) {
  const driver = await readDriver(key, { row, backendId });
  ...
  const got = await driver.get(key, { range: range ? rangeHeader : null });
...
export async function softDeleteAssetFile(assetId) {
  ...
  try {
    ...
    if (row.storage_key) invalidateKey(row.storage_key, row.storage_backend ?? null);
    await safeUpdateTag(`asset:${assetId}`);
