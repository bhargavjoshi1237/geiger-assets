...
import { memberRole } from "@/lib/storage/auth";
...
export async function proxyStore({ projectId, uploadJobId, filename, contentType, bytes, assetId, name, type, folder, tags, checksum }) {
  ...
  try {
    ...
    key = assetId
      ? await nextVersionKey({ projectId, assetId, filename })
      : stagingKey({ projectId, uploadJobId, filename });
  } catch {
    return { error: "bad_request" };
  }
  const fileType = assetTypeForContentType(contentType, filename) || "image";
...
export async function authorizeKey(key, sessionUserId) {
  ...
  try {
    ...
    const m = await memberRole(sb, parsed.projectId, sessionUserId);
    if (!m.roleKey && !m.open) return null;
    return parsed;
  } catch (e) {
...
export async function batchSign(keys, sessionUserId) {
  ...
  if (cfg && !cfg.presignedReads) {
    ...
    for (const key of list) {
      ...
      urls[key] = ok ? `${base}/api/storage/file?key=${encodeURIComponent(key)}` : null;
    }
