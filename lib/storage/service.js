...
function toAssetInsert({ projectId, key, head, checksum, name, type, folder, tags, filename, contentType, storageBucket, poolFileId, poolUrl }) {
  return {
    project_id: projectId,
    name: name || filename,
...
export async function commitUpload({ uploadJobId, key, assetId, checksum, name, type, folder, tags, parts, response, providerKey }) {
  ...
  try {
    ...
    if (assetId) {
      ...
      const { data, error } = await sb.schema("assets").from("assets").insert(
        toAssetInsert({ projectId, key, head, checksum, name, type, folder, tags, filename, contentType: head.contentType, storageBucket, ...(pooled ? { poolFileId, poolUrl } : {}) })
      ).select("*").single();
      if (error) {
        console.error("[storage.commit]", error.message);
        await markJob(uploadJobId, { status: "failed", error: error.message });
...
export async function proxyStore({ projectId, uploadJobId, filename, contentType, bytes, assetId, name, type, folder, tags, checksum, quality }) {
  ...
  return commitUpload({ uploadJobId, key, assetId, checksum, name: name || effectiveFilename, type, folder, tags });
}
