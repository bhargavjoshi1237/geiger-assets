...
// Mirrors VARIANTS in lib/media/derive.js. Duplicated rather than imported
// because derive.js is server-only (it throws in the browser).
const VARIANT_NAMES = ["thumb", "preview", "poster"];

// Delivery URL for a derived size. The route negotiates format from Accept and
// streams the original when no derivative exists, so this is safe for any asset
// — including rows uploaded before derivation shipped. Pass `format` only to
// override negotiation; an unknown variant returns "" rather than a 404 URL.
export function assetVariantUrl(assetId, variant, { format } = {}) {
  if (!assetId) return "";
  const name = typeof variant === "string" ? variant.trim().toLowerCase() : "";
  if (!VARIANT_NAMES.includes(name)) return "";
  const query = format ? `?format=${encodeURIComponent(format)}` : "";
  return api(`/api/media/${assetId}/${name}${query}`);
}
