...
export async function uploadAsset(file, { projectId, assetId, name, type, status, folder, tags, quality, onProgress, onPhase, onError, signal } = {}) {
  ...
  async function commit({ uploadJobId, key, filename, checksum, fromProxy, quality: preset, parts, response, providerKey }) {
    phase("committing");
    if (fromProxy) {
  ...
  try {
    ...
    if (backend === "pool" && mode && ticket) {
      ...
      report(92);
      return commit({
        uploadJobId, key, filename, checksum, quality: preset,
