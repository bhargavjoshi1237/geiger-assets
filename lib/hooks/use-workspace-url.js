...
// for a clean URL.
//
// Canonical schema:  /project/<projectId>/<tab-slug>?asset=ast_123&section=relationships&workflow=wf_123
...
//   - workflow → id of the open workflow in All Workflows. None ⇒ omitted.
...
export function useWorkspaceUrl() {
  ...
  const buildUrl = useCallback(
    ...
    [projectId, tab, assetId, section, workflowId, pathname, onProjectRoute],
  );
  ...
  // Switching workspace tabs exits any open asset/section/workflow.
  const setTab = useCallback(
    (next) => apply({ tab: next, asset: null, section: null, workflow: null }),
    [apply],
  );
