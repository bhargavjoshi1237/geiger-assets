...
// symmetry with probe/optimize but never trusted. deriveTransform is the same
// primitive for an ad-hoc size (the URL-transform delivery route) and shares
// renderRaster with deriveVariant, so both obey one input ceiling, one
// classifier and one no-upscale rule.
//
...
// WebP is the default for all three — universal browser decode, always present
// via sharp's libwebp, ~25-35% smaller than JPEG at matched quality, and
// alpha-preserving (JPEG would flatten transparent PNGs onto black/white).
//
// AVIF is available as an opt-in override but is deliberately NOT the default
// here: measured on this toolchain it encodes 4-5x slower than WebP (poster
// 1165ms vs 276ms), which is unacceptable multiplied across a commit-time
// fan-out. The delivery path derives AVIF on demand instead, once per asset,
// and caches the result — see lib/media/variants.js.
export const DERIVE_FORMATS = Object.freeze({
  webp: Object.freeze({ contentType: "image/webp", ext: "webp" }),
  avif: Object.freeze({ contentType: "image/avif", ext: "avif" }),
});
...
// The one place raw bytes reach sharp. Both the named-variant path and the
// ad-hoc transform path funnel through here so classification, the input
// ceiling, EXIF rotation and the no-upscale rule cannot drift apart between
// them. Returns null on every failure — callers treat "cannot derive" and
// "not an image" identically.
async function renderRaster(input, { width, height, fit, quality, fmtName }) {
  ...
  // small originals (a 100 px icon stays 100 px, just re-encoded) — which is
  // also what stops a transform spec asking for w_1920 of a tiny original
  // from turning into an expensive, pointless upscale.
  // SVG takes the same path: sharp rasterises the vector to the requested
  // box and the bytes returned are always raster — never the SVG original.
  const resize = { fit, withoutEnlargement: true };
  if (Number.isInteger(width) && width > 0) resize.width = width;
  if (Number.isInteger(height) && height > 0) resize.height = height;
  // Transparent padding for the fits that pad (sharp would otherwise letterbox
  // onto opaque black); ignored entirely by the fits that do not.
  resize.background = { r: 0, g: 0, b: 0, alpha: 0 };

  const pipeline = sharp(input, { animated: false }).rotate().resize(resize);
  const encoded = fmtName === "avif"
    ? pipeline.avif({ quality, effort: 4 })
    : pipeline.webp({ quality, effort: 4 });
  const { data, info } = await encoded.toBuffer({ resolveWithObject: true });
  if (!data || data.length === 0) return null;
  const outWidth = Number.isInteger(info?.width) && info.width > 0 ? info.width : null;
  const outHeight = Number.isInteger(info?.height) && info.height > 0 ? info.height : null;
  if (outWidth === null || outHeight === null) return null;
  return {
    bytes: data,
...
export async function deriveVariant(buffer, variant, { contentType, filename, format } = {}) {
  ...
  try {
    ...
    if (!spec) return null;
    // An unknown format falls back to the variant default rather than failing:
    // a caller asking for a codec we cannot encode should still get bytes.
    const fmtName = typeof format === "string" && Object.hasOwn(DERIVE_FORMATS, format.trim().toLowerCase())
      ? format.trim().toLowerCase()
      : spec.format;
    const input = toInputBuffer(buffer);
    if (!input) return null;
    return await renderRaster(input, {
      width: spec.width,
      height: spec.height,
      fit: spec.fit || "inside",
...
// Ad-hoc resize for the URL-transform delivery route. The caller is expected
// to have run the spec through lib/media/transform.js first — this function
// deliberately does no allowlisting of its own, because the allowlist is a
// property of the public URL surface, not of the imaging primitive. Never
// throws (null on any failure), same as deriveVariant.
export async function deriveTransform(buffer, { width, height, fit, quality, format } = {}) {
  try {
    const fmtName = typeof format === "string" && Object.hasOwn(DERIVE_FORMATS, format.trim().toLowerCase())
      ? format.trim().toLowerCase()
      : "webp";
    const w = Number.isInteger(width) && width > 0 ? width : null;
    const h = Number.isInteger(height) && height > 0 ? height : null;
    if (w === null && h === null) return null;
    const q = Number.isInteger(quality) && quality > 0 && quality <= 100 ? quality : 75;
    const input = toInputBuffer(buffer);
    if (!input) return null;
    return await renderRaster(input, {
      width: w,
      height: h,
      fit: typeof fit === "string" && fit ? fit : "inside",
      quality: q,
      fmtName,
    });
  } catch (e) {
    console.error("[media.derive.transform]", e?.message || e);
    return null;
  }
}

export async function deriveAll(buffer, { contentType, filename, variants, format } = {}) {
  try {
    let names;
    try {
    ...
    for (const name of names) {
      try {
        const d = await deriveVariant(buffer, name, { contentType, filename, format });
        if (d) out[name] = d;
      } catch {
